...
const Player = () => {
  ...
  const { calculateChapterTime, enrolledCourses, backendUrl, getToken, fetchUserEnrolledCourses, userData } = useContext(AppContext);
  ...
  const markLectureAsCompleted = async (lectureId)=>{
    try {
      ...
      const { data } = await axios.get(backendUrl + '/api/user/update-course-progress' ,{courseId, lectureId}, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleRate = async(rating)=>{
    try {
      const token = await getToken();
      const { data } = await axios.get(backendUrl + '/api/user/add-rating' ,{courseId, rating}, { headers: { Authorization: `Bearer ${token}` } })
